...
def test_get_single_post(api_base_url):
...
def test_filter_posts_by_user(api_base_url):
    response = requests.get(f"{api_base_url}/posts", params={"userId": 1})
...
def test_update_post(api_base_url):
    ...
    response = requests.put(f"{api_base_url}/posts/1", json=updated_post)
    data  = response.json()
    assert response.status_code == 200
...
def test_post_not_found(api_base_url):
    response = requests.get(f"{api_base_url}/posts/9999")
    assert response.status_code == 404, f"Expected 404, got {response.status_code}"

def test_response_headers(api_base_url):
    response = requests.get(f"{api_base_url}/posts/1")
