...
def test_filter_posts_by_user(api_url):
    response = requests.get(f"{api_url}/posts", params={"userId": 1})
...
def test_post_not_found(api_url):
    response = requests.get(f"{api_url}/posts/9999")
    assert response.status_code == 404, f"Expected 404, got {response.status_code}"

def test_response_headers(api_url):
    response = requests.get(f"{api_url}/posts/1")
